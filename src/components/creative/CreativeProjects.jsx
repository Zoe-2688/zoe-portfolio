...
function CircuitLines({ hovered, reduceMotion }) {
  return (
    <svg className="absolute inset-0 pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none"
      style={{ width: '100%', height: '100%', opacity: hovered ? 0.6 : 0.25, transition: reduceMotion ? 'none' : 'opacity 300ms ease' }}>
    ...
    >
      ...
      <div className="flex flex-col flex-1 gap-3 p-6">
        ...
        {isLarge && project.longDescription && (
          <p className="text-[13px] text-white leading-relaxed" style={{ opacity: 0.75 }}>{project.longDescription}</p>
        )}
        {isLarge && project.metric && (
          <div className="flex items-start gap-2 rounded-md px-3 py-2 mt-1"
        ...
        {!isLarge && project.longDescription && (
          <div style={{
            ...
            {project.metric && (
              <div className="flex items-start gap-2 rounded-md px-2.5 py-1.5 mt-2"
                style={{ backgroundColor: 'rgba(232,160,144,0.08)', border: '1px solid rgba(232,160,144,0.2)' }}>
