...
const TITLE_COLORS = ['#e8a090', '#00d4ff', '#ffffff', '#e8a090', '#00d4ff', '#ffffff', '#e8a090', '#00d4ff']
  ...
  return [
    {
      ...
      metricIcon: '💡', tags: ['Figma', 'UX/UI', 'Accesibilidad', 'WCAG'], hasCaseStudy: true,
    },
    {
      id: 'johnny-rockets', image: johnnyRocketsImg, title: 'Johnny Rockets Chile',
      ...
      metricIcon: '🇨🇱', tags: ['HTML', 'Tailwind CSS', 'JavaScript'], hasCaseStudy: true,
    },
    {
      id: 'flagship', image: flagshipImg, title: 'Flagship CMS',
      ...
      metricIcon: '📘', tags: ['Figma', 'Canva', 'B2B'], hasCaseStudy: true,
    },
  ]
}

function CircuitNode({ corner, hovered, reduceMotion }) {
  const cornerStyles = { 'top-left': 'top-2 left-2', 'top-right': 'top-2 right-2', 'bottom-left': 'bottom-2 left-2', 'bottom-right': 'bottom-2 right-2' }
  return (
    <div className={`absolute ${cornerStyles[corner]} pointer-events-none`}>
      <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: '#00d4ff', opacity: hovered ? 0.9 : 0.5, boxShadow: hovered ? '0 0 10px rgba(0,212,255,0.7)' : '0 0 3px rgba(0,212,255,0.3)', transition: reduceMotion ? 'none' : 'opacity 300ms ease, box-shadow 300ms ease' }} />
    </div>
  )
}
...
function CircuitLines({ hovered, reduceMotion }) {
  return (
    <svg className="absolute inset-0 pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none" style={{ width: '100%', height: '100%', opacity: hovered ? 0.6 : 0.25, transition: reduceMotion ? 'none' : 'opacity 300ms ease' }}>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 gap-3 p-6">
        ...
        <div className="self-start mt-auto cursor-pointer" style={{ fontFamily: 'monospace', fontSize: '18px', fontWeight: 600, marginTop: '1rem' }}
          onMouseEnter={() => setCmdHovered(true)} onMouseLeave={() => setCmdHovered(false)} onClick={handleVerCaso}>
          <span style={{ transition: reduceMotion ? 'none' : 'color 200ms ease', color: cmdHovered ? '#e8a090' : '#00d4ff' }}>
            <span style={{ textShadow: cmdHovered ? '0 0 8px rgba(232,160,144,0.6), 0 0 16px rgba(232,160,144,0.3)' : '0 0 6px rgba(0,212,255,0.5), 0 0 12px rgba(0,212,255,0.2)', transition: reduceMotion ? 'none' : 'text-shadow 200ms ease' }}>
              {verCaso}{' '}
            </span>
            <span style={{ display: 'inline-block', transition: reduceMotion ? 'none' : 'transform 200ms ease', transform: cmdHovered ? 'translateX(4px)' : 'translateX(0)' }}>→</span>
          </span>
        </div>
      </div>
    </div>
...
function CreativeProjects({ onOpenCaseStudy }) {
  ...
  const titleChars = (t.projects.title || 'PROYECTOS').split('')
